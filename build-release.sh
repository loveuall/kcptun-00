...
for os in ${OSES[@]}; do
	for arch in ${ARCHS[@]}; do
		...
		env CGO_ENABLED=0 GOOS=$os GOARCH=$arch go build -ldflags "$LDFLAGS" -o client_${os}_${arch}${suffix} github.com/loveuall/kcptun/cmd/client
		env CGO_ENABLED=0 GOOS=$os GOARCH=$arch go build -ldflags "$LDFLAGS" -o server_${os}_${arch}${suffix} github.com/loveuall/kcptun/cmd/server
